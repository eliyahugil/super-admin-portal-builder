...
-- מחיקת הנתונים הקודמים אם קיימים (לנקות שגיאות)
DELETE FROM public.employee_notes WHERE business_id IN (SELECT id FROM public.businesses WHERE name = 'חברת הדמו טק בעמ');
DELETE FROM public.business_module_config WHERE business_id IN (SELECT id FROM public.businesses WHERE name = 'חברת הדמו טק בעמ');
DELETE FROM public.employees WHERE business_id IN (SELECT id FROM public.businesses WHERE name = 'חברת הדמו טק בעמ');
DELETE FROM public.branches WHERE business_id IN (SELECT id FROM public.businesses WHERE name = 'חברת הדמו טק בעמ');
DELETE FROM public.businesses WHERE name = 'חברת הדמו טק בעמ';
...
BEGIN
    ...
    -- יצירת עובדים לדוגמה (תוקן הערך מ-contract ל-contractor)
