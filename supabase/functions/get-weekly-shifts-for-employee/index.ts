...
serve(async (req) => {
  ...
  try {
    const { token, week_start_date } = await req.json();
    ...
    console.log('🔍 Getting weekly shifts for token:', token, 'week:', week_start_date);
    ...
    // Calculate week dates based on request or token default
    let weekStart = week_start_date || tokenData.week_start_date;
    let weekEnd;
